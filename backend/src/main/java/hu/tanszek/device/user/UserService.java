 ...
 *
 * <p>A UserService a CRUD műveleteken kívül a password change és a deactivation flow-t kezeli. Az
 * authentication a {@link hu.tanszek.device.auth.LoginService}-ben van.
 *
 * @see LoginService
...
public class UserService {
   ...
   *
   * <p>A metódus ellenőrzi a currentPassword-t (biztonsági okokból), és ha helyes, beállítja az új
   * jelszót (Argon2id hash), törli a must_change_password flag-et, és frissíti a
   * password_changed_at timestampet.
   *
  ...
  public void changePassword(Long userId, String currentPassword, String newPassword) {
    ...
    if (newPassword == null || newPassword.length() < 12) {
      throw new BusinessValidationException(
          "passwordChangeTooShort", "New password must be at least 12 characters");
    }

    // 3. New password != current password
    if (passwordEncoder.matches(newPassword, user.getPasswordHash())) {
      throw new BusinessValidationException(
          "passwordChangeSameAsOld", "New password must be different from current");
    }
   ...
   * User deaktiválása.
   *
   * <p>A deaktivált user nem tud belépni ({@code active = false}). Az aktív refresh tokenjei
   * automatikusan revokeolódnak, így a meglévő session-jei is érvénytelenítődnek.
   *
   * @param userId a user ID-ja
   * @throws ResourceNotFoundException ha a user nem található
   */
   ...
   *
   * <ul>
   *   <li>{@code role} — Role entitás (lookup a {@code roles} táblából név alapján)
   *   <li>{@code officeLocationId} — irodai location (opcionális, lehet null = törölve)
   *   <li>{@code active} — boolean (deaktiváláshoz)
   * </ul>
   *
   * <p>Az email és a jelszó ezen a metóduson keresztül <b>nem</b> módosítható — azok külön
   * endpointokon mennek (admin reset email, user password change).
   *
   * <p>Audit log bejegyzés generálódik ({@code @AuditTarget}).
   *
   * @param userId a user ID-ja
   * @param roleName új role név, pl. "ROLE_ADMIN" (null = nem változik)
   * @param officeLocationId új office location ID (null = nem változik; {@code
   *     clearOfficeLocation=true} esetén törölhető)
    ...
    if (roleName != null) {
      Role role =
          roleRepository
              ...
              .orElseThrow(
                  () ->
                      new BusinessValidationException("invalidRole", "Unknown role: " + roleName));
      user.setRole(role);
    }
    if (clearOfficeLocation) {
      ...
      Location office =
          locationRepository
              ...
              .orElseThrow(
                  () -> new ResourceNotFoundException("Location not found: " + officeLocationId));
