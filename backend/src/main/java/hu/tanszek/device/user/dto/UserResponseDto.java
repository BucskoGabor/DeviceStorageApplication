 ...
 *
 * <p>Az {@code emailEncrypted} mezőt SOHA nem adjuk vissza plaintext-ben —
 * a DTO csak az {@code emailMasked} (pl. {@code a***@tanszek.local}) formátumot
 * küldi, amit a CryptoService.decrypt + maszkolás generál.
 *
 * <p>A plain email NEM kerül a wire-ba — defense-in-depth elv: a DB-ben
 * AES-GCM titkosítva van, és a kliens csak maszkolt formátumban látja.
 *
 * <p>Az {@code officeLocationSummary} egy egyszerűsített nested DTO (id + name + type),
 * nem a teljes {@code Location} entitás — így a Hibernate LAZY proxy nem kerül
 * szerializálásra (amit a Jackson nem tud kezelni ByteBuddyInterceptor exception-nel).
 */
public record UserResponseDto(
    ...
    public record OfficeLocationSummary(
            Long id,
            String name,
            String type
    ) {}
    ...
    public static UserResponseDto fromEntity(AppUser user, CryptoService cryptoService) {
        ...
        if (user.getOfficeLocation() != null) {
            try {
                officeSummary = new OfficeLocationSummary(
