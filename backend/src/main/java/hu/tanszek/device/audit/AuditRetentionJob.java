 ...
 *       /var/backups/archive/audit/YYYY/ mappába (audit_archive_data volume mount)
 *   <li><b>5+ éves rekordok</b> végleges törlése a DB-ből
 * </ul>
 *
 * <p>Threshold-ek az {@code audit.retention-years} (5) config-ból jönnek.
 *
 * <p>Email alert küldése a ScheduledJobMonitoring wrapper-en keresztül, ha bármi hiba. A cleanup
 * nem retry-ol automatikusan, a következő heti futás újrapróbálkozik.
 */
@Slf4j
@Component
...
public class AuditRetentionJob {
   ...
   *
   * <p>1. 1+ éves rekordok exportálása NDJSON.gz formátumban 2. 5+ éves rekordok törlése
   */
   ...
   *
   * <p>Fájl formátum: {archive_path}/YYYY/audit-YYYY-MM-DD.ndjson.gz (a YYYY a retention év, az
   * MM-DD a backup dátum).
   */
  private void archiveOldRecords(Instant cutoff) {
    ...
    for (AuditLog record : recordsToArchive) {
      int year =
          LocalDate.ofInstant(record.getTimestamp(), java.time.ZoneId.systemDefault()).getYear();
  ...
  private Path writeArchiveFile(int year, List<AuditLog> records) throws IOException {
    String filename =
        "audit-" + LocalDate.now().format(DateTimeFormatter.ISO_LOCAL_DATE) + ".ndjson.gz";
