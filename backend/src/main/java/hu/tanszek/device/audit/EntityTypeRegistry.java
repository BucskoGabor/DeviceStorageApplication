 ...
 *
 * <p>A {@code audit_logs.entity_type} mező String ("Device", "User", stb.),
 * és a rollback-hez tudni kell, melyik repository-t kell használni az adott
 * típus betöltéséhez. Ez az osztály centralizálja a típus→repository mappinget.
 *
 * <p>Támogatott típusok:
 * <ul>
 *   <li>Device → DeviceRepository.findById()</li>
 *   <li>User → AppUserRepository.findById()</li>
 *   <li>Location → LocationRepository.findById()</li>
 *   <li>Assignment → DeviceAssignmentRepository.findById()</li>
 *   <li>Software → SoftwareRepository.findById()</li>
 *   <li>Attachment → DeviceAttachmentRepository.findById()</li>
 * </ul>
 */
@Component
...
public class EntityTypeRegistry {
     ...
     *
     * <p>A JSON-ba konvertálás az entity mezőiből történik. A {@code changes_json}
     * mezőben tárolt before/after diff-et hasonlítjuk össze.
     *
    ...
    public Map<String, Object> toJsonMap(Object entity) {
        ...
        if (entity instanceof Device device) {
            ...
            map.put("fromLocationId", assignment.getFromLocation() != null ? assignment.getFromLocation().getId() : null);
            map.put("toLocationId", assignment.getToLocation() != null ? assignment.getToLocation().getId() : null);
            map.put("fromUserId", assignment.getFromUser() != null ? assignment.getFromUser().getId() : null);
     ...
     *
     * <p>Frissíti az entitás mezőit a JSON map alapján. Csak azokat a mezőket
     * frissíti, amelyek a map-ben szerepelnek (before/after diff-ek).
     *
    ...
    public void applyJsonMap(Object entity, Map<String, Object> fields) {
        ...
        if (entity instanceof Device device) {
            ...
            if (fields.containsKey("inventoryNumber")) device.setInventoryNumber((String) fields.get("inventoryNumber"));
            if (fields.containsKey("status") && fields.get("status") != null) {
                device.setStatus(hu.tanszek.device.device.entity.DeviceStatus.valueOf(fields.get("status").toString()));
            }
        } else if (entity instanceof AppUser user) {
     ...
     *
     * <p>A map-et a konstruktor után töltjük fel, hogy a dependency injection
     * befejeződjön.
     */
