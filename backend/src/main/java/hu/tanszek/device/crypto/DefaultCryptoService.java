 ...
 *
 * <p>Az AES-256 kulcs az alkalmazás indításakor töltődik be a {@code CRYPTO_AES_KEY} env var-ból
 * (base64 kódolás). A {@link PostConstruct} fail-fast viselkedéssel rendelkezik: ha a kulcs hossza
 * nem 32 byte (256 bit), az alkalmazás indítása sikertelen.
 *
 * <p>A {@link #encrypt(String)} metódus minden hívásban új random 12-byte IV-t generál
 * (SecureRandom-ból), és a IV-t a ciphertext prefix-eként tárolja ({@code iv:ciphertext} base64
 * formátumban). Ez biztosítja, hogy azonos plaintext különböző ciphertext-et eredményez (IND-CPA
 * biztonság).
 *
 * <p>A GCM mód egy 128-bit authentication tag-et is generál, amelyet a ciphertext végéhez fűz — ez
 * biztosítja az integritás-védelmet (tampering esetén a {@link #decrypt(String)} {@link
 * CryptoException}-t dob).
 */
@Slf4j
@Service
