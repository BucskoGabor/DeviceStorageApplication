 ...
 *
 * <ol>
 *   <li>RequestIdFilter — UUID request_id generálás (MDC-be)
 *   <li>RateLimitFilter — Bucket4j per-IP limit (CsrfFilter ELŐTT)
 *   <li>CsrfFilter — CSRF token check (CookieCsrfTokenRepository, /api/auth/login és
 *       /api/auth/refresh kivétel)
 *   <li>JwtAuthenticationFilter — Bearer token validáció, SecurityContext
 *   <li>UsernamePasswordAuthenticationFilter — csak /api/auth/login (form login)
 *   <li>ExceptionTranslationFilter — Spring Security default
 *   <li>AuthorizationFilter — @RequirePermission / URL-alapú permission check
 * </ol>
 */
@Configuration
@EnableWebSecurity
...
public class SecurityConfig {
  ...
  /**
   * Argon2PasswordEncoder bean — memory-hard, GPU/ASIC-resistant, OWASP 2024+ ajánlás. Paraméterek:
   ...
   *
   * <p>A {@link RoleHierarchyImpl} a Spring Security-ben egy ROLE_ADMIN user örökli a ROLE_TEACHER
   * és ROLE_STUDENT összes permission-jét (ahol a RoleHierarchy bean definiálja a "tartalmazza"
   * relációt).
   *
   * <p>A {@code RoleHierarchyVoter} ezt használja a permission check-eknél.
   *
   * <p>Ha új role jön a rendszerbe (admin UI-ból), itt kell frissíteni és újraindítani a
   * backend-et.
   */
  @Bean
   ...
   *
   * <ul>
   *   <li>CSRF aktív (CookieCsrfTokenRepository, /api/auth/login és /api/auth/refresh kivétel)
   *   <li>Session: STATELESS (JWT alapú)
   *   <li>CORS kikapcsolva (same-origin — Vite proxy / Nginx reverse proxy)
   *   <li>Public: /api/auth/login, /api/auth/refresh, /api/auth/logout, /actuator/health,
   *       /actuator/info
   *   <li>Authenticated: minden más
   *   <li>Filter lánc: RequestIdFilter → RateLimitFilter → CsrfFilter → JwtAuthenticationFilter
   * </ul>
   */
  @Bean
