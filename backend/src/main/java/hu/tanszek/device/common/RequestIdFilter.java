 ...
 *
 * <p>Ha a kliens küld X-Request-ID headert (pl. distributed tracing-hez), elfogadjuk.
 * Egyébként UUID-t generálunk. A request_id a response X-Request-ID headerben
 * visszaadódik, hogy a kliens is lássa (debug, support ticket-ek).
 *
 * <p>MDC (Mapped Diagnostic Context) propagálódik a structured JSON log-okba
 * (lásd logback-spring.xml): minden log bejegyzés tartalmazza a request_id-t.
 *
 * <p>Filter pozíció: Security Filter Chain legeleje (RateLimitFilter előtt),
 * hogy a request_id minden log-ban megjelenjen, még a security check előtt.
 */
@Component
