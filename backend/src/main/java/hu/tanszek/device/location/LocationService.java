...
public class LocationService {
    ...
    /**
     * Maximális fa-mélység, amit a {@link #buildTree()} visszaad.
     * Védelmet nyújt végtelen rekurzió vagy rossz adat ellen.
     */
     ...
     *
     * <p>A tree a root node-októl (parent == null) indul, és rekurzívan járja be
     * a children kollekciót. A {@link Location#getChildren()} kollekció
     * {@code LAZY} fetch, ezért a metódus {@code @Transactional(readOnly = true)}-ben fut.
     *
     * <p>A {@link #MAX_TREE_DEPTH} (10) korlát védi a végtelen rekurziótól.
     * Ha egy adott hierarchia mélyebb, a level {@code depth = MAX_TREE_DEPTH} node
     * üres children listát kap — a frontend így is jelzi, hol van a határ.
     *
     * <p>A DB szintű ciklusok ellen a {@link #visitedIds} set véd —
     * bár a {@link #validateNoCycle} ezt megakadályozza create/update során,
     * a védelem defense-in-depth.
     *
    ...
    public List<LocationTreeDto> buildTree() {
        ...
        return roots.stream()
                .map(root -> buildNode(root, null, 0, new java.util.HashSet<>()))
                .toList();
    }

    private LocationTreeDto buildNode(Location location, Long parentId, int depth, java.util.Set<Long> visitedIds) {
