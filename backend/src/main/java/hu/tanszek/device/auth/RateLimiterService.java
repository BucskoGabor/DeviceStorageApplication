 ...
 * <ul>
 *   <li><b>per-IP:</b> 5 próba / perc (gyors brute-force védelem)</li>
 *   <li><b>per-email:</b> 10 próba / óra (lassabb targeted attack védelem)</li>
 * </ul>
 *
 * <p>Bucket-ek {@code ConcurrentHashMap}-ben tárolódnak, kulcs az IP-cím
 * vagy az email SHA-256 hash-e. Régi entry-k cleanup-ja a memory leak
 * ellen (Task 5.5-ben implementálva, ha a rendszer hosszabb ideig fut).
 *
 * <p>HA deployment esetén Redis-backed Bucket4j-re kell váltani
 * (Future Work).
 */
@Slf4j
@Component
...
public class RateLimiterService {
    ...
    /** Per-IP: 5 próba / perc */
    private static final Bandwidth PER_IP_BANDWIDTH = Bandwidth.classic(
            5,
            Refill.intervally(5, Duration.ofMinutes(1))
    );

    /** Per-email: 10 próba / óra */
    private static final Bandwidth PER_EMAIL_BANDWIDTH = Bandwidth.classic(
            10,
     ...
     * @return true ha a kérés átmehet (bucket van szabad token),
     *         false ha rate limit exceeded
     */
    public boolean tryConsumePerIp(String ip) {
        if (ip == null || ip.isBlank()) {
            return true; // Nincs IP, nem tudunk rate limit-et alkalmazni (proxy mögött?)
        }
        Bucket bucket = perIpBuckets.computeIfAbsent(ip, k -> Bucket.builder()
                .addLimit(PER_IP_BANDWIDTH)
                .build());
    ...
    public boolean tryConsumePerEmail(String email) {
        ...
        Bucket bucket = perEmailBuckets.computeIfAbsent(emailHash, k -> Bucket.builder()
                .addLimit(PER_EMAIL_BANDWIDTH)
                .build());
        boolean allowed = bucket.tryConsume(1);
        if (!allowed) {
