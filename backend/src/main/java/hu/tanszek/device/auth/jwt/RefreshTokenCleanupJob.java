 ...
 *
 * <p>Törli a 7+ napos lejárt vagy revoked tokeneket. A grace period debug célokat szolgál — ha egy
 * user logoutolt, a tokenje 7 napig megmarad a DB-ben, hogy az esetleges audit log-okban
 * trace-elhető legyen.
 *
 * <p>Ütemezés: {@code cron = "0 0 4 * * *"} (minden nap 04:00-kor, UTC).
 *
 * <p>Hiba esetén email alert küldése a {@link ScheduledJobMonitoring}-on keresztül az {@code
 * ALERT_EMAIL_RECIPIENT}-nek.
 */
@Slf4j
@Component
...
public class RefreshTokenCleanupJob {
   ...
   *
   * <ul>
   *   <li>Lejárt tokenek ({@code expires_at < now}) — azonnal
   *   <li>Revoked tokenek ({@code revoked = true AND created_at < now - 7 days}) — grace után
   * </ul>
   */
