...
/**
 * AuthProviderFactory — a {@code configs.AUTH_PROVIDER} alapján kiválasztja
 * az aktív {@link AuthProvider} implementációt.
 *
 * <p>A factory a Spring összes {@link AuthProvider} bean-jét összegyűjti
 * (LocalAuthProvider, StubAdAuthProvider, stb.) és azonosítja őket a
 * {@link AuthProvider#getProviderId()} alapján.
 *
 * <p>Startup-kor beolvassa a {@code AUTH_PROVIDER} értéket a {@code configs}
 * táblából. Ha az érték változik runtime-ban (pl. admin updateeli az adatbázist),
 * a {@code @RefreshScope} miatt a factory újra betöltődik.
 *
...
public class AuthProviderFactory {
    ...
    /**
     * A factory inicializálása — provider-ök map-be szervezése provider id alapján.
     * A {@code @PostConstruct} a Spring DI után fut le.
     */
    @PostConstruct
    public void init() {
        this.providerMap = providers.stream()
                .collect(Collectors.toMap(
                        ...
                        (existing, replacement) -> {
                            log.warn("Duplicate provider id: {} — keeping first",
    ...
    /**
     * Az aktív {@link AuthProvider} visszaadása a {@code configs.AUTH_PROVIDER}
     * értéke alapján.
     *
    ...
    public AuthProvider getActiveProvider() {
        ...
        if (provider == null) {
            throw new NoSuchElementException(
                    "No AuthProvider available for id '" + providerId + "' or fallback '" + FALLBACK_PROVIDER + "'"
            );
        }
    ...
    private String resolveProviderId() {
        return configRepository.findByKey(CONFIG_KEY)
                .map(Config::getValue)
