 ...
 *
 * <p>Az aktuális kulcs a {@link JwtProperties}-ből jön (env var: JWT_KID_ACTIVE). A {@code kid}
 * (Key ID) header mezőt használja a rotációhoz — ha a token {@code kid} megegyezik az aktív kulcs
 * {@code kid}-jével, az aktív kulccsal validálódik; ha a grace period-on belül van és a {@code kid}
 * a previous kulcshoz tartozik, a previous kulccsal is validálható.
 *
 * <p>Access token TTL: {@link JwtProperties#getAccessTokenTtlMin()} perc. Refresh token a DB-ben
 * tárolódik ({@code refresh_tokens} tábla), nem JWT.
 */
@Slf4j
@Service
public class JwtTokenProvider {
  ...
  public Claims validateToken(String token) {
    return Jwts.parser().verifyWith(activeKey).build().parseSignedClaims(token).getPayload();
  }

  /**
   * Token validáció grace period-dal — ha a token invalid az aktív kulccsal, de a grace period-on
   * belül vagyunk, a previous kulccsal is megpróbálja.
   *
   * @param token a JWT string
   * @return Claims
