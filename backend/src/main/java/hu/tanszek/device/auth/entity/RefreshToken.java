 ...
 *
 * <p>A refresh tokent SHA-256 hash-szel tároljuk ({@code tokenHash}). A plain token soha nincs
 * adatbázisban — csak a HttpOnly Secure SameSite=Strict cookie-ban a kliensnél.
 *
 * <p>Rotation chain:
 *
 * <ul>
 *   <li>Minden refresh híváskor új refresh token generálódik
 *   <li>A régi token {@code revoked = true} lesz, és {@code replacedById} mutat az új tokenre
 *   <li>Ha egy revoked tokennel próbálkoznak, az egész chain revokeolódik (reuse detection)
 * </ul>
 *
 * <p>Cleanup: napi 04:00-kor {@code @Scheduled} job törli a 7+ napos lejárt/revoked tokeneket.
 *
