 ...
 *
 * <p>Az authenticate metódus az {@link AuthProviderFactory} által szolgáltatott aktív provider-re
 * delegál (LOCAL/AD), majd sikeres hitelesítés esetén ellenőrzi az Argon2 hash paramétereit, és
 * szükség esetén transparent újrahasheli + DB-be írja.
 *
 * <p>Rehash flow:
 *
 * <ol>
 *   <li>Sikeres authenticate() a provider-től (LocalAuthProvider.matches())
 *   <li>passwordEncoder.upgradeEncoding(user.passwordHash) — true ha a hash paraméterei elavultak
 *   <li>Ha true: passwordEncoder.encode(rawPassword) → user.passwordHash = newHash
 *   <li>user.passwordChangedAt = NOW() — first-login reset
 *   <li>user.mustChangePassword = false — first-login flag törlése
 *   <li>appUserRepository.save(user) — egy tranzakcióban
 * </ol>
 */
@Slf4j
@Service
...
public class LoginService {
    ...
    if (authentication.getPrincipal() instanceof String emailHash) {
      ...
      AppUser user =
          appUserRepository
              ...
              .orElseThrow(
                  () -> new BadCredentialsException("User disappeared during authentication"));
   ...
   *
   * <p>Ez a metódus a Spring Security {@code Argon2PasswordEncoder.upgradeEncoding()} metódusát
   * használja, ami ellenőrzi a hash beágyazott paramétereit (memory, iterations, parallelism) a
   * konfigurációban beállított policy-hez képest.
   *
  ...
  private void maybeRehashPassword(AppUser user, String rawPassword) {
    if (passwordEncoder.upgradeEncoding(user.getPasswordHash())) {
      log.info(
          "Upgrading password hash for user {} (old params → new params)", user.getEmailHash());
