 ...
 *
 * <p>Betölti az AppUser-t az email_hash alapján (a JwtTokenProvider subject mezője email_hash, NEM
 * email), és visszaadja a UserDetails-t a role + permission authorities-kkal.
 *
 * <p>A security kontextusba betöltött authorities:
 *
 * <ul>
 *   <li>ROLE_ADMIN / ROLE_TEACHER / ROLE_STUDENT — a user role-ja
 *   <li>DEVICE_READ / USER_MANAGE / stb. — a role.permission-ök + user.permission-ök uniója
 * </ul>
 */
@Service
...
public class CustomUserDetailsService implements UserDetailsService {
  ...
  public UserDetails loadUserByUsername(String emailHash) throws UsernameNotFoundException {
    AppUser user =
        appUserRepository
            ...
            .orElseThrow(
                () ->
                    new UsernameNotFoundException("User not found with email_hash: " + emailHash));
  ...
  private Collection<GrantedAuthority> getAuthorities(AppUser user) {
    ...
    // Role permission-ök
    Stream<GrantedAuthority> rolePerms =
        user.getRole().getPermissions().stream().map(p -> new SimpleGrantedAuthority(p.getName()));

    // User-specifikus permission-ök
    Stream<GrantedAuthority> userPerms =
        user.getPermissions().stream().map(p -> new SimpleGrantedAuthority(p.getName()));
