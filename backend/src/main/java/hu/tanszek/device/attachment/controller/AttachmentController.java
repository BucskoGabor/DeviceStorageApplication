 ...
 *
 * <ul>
 *   <li>GET /api/devices/{deviceId}/attachments — lista
 *   <li>POST /api/devices/{deviceId}/attachments — feltöltés (multipart)
 *   <li>DELETE /api/attachments/{id} — törlés
 *   <li>GET /api/attachments/{id}/file?inline=true|false — letöltés vagy inline preview
 * </ul>
 */
@RestController
...
@Tag(
    name = "Attachment",
    description =
        "Eszköz mellékletek (képek, PDF, Office fájlok) feltöltése, listázása, letöltése és törlése")
public class AttachmentController {
  ...
  @Operation(
      summary = "Eszköz mellékletei",
      description = "Visszaadja az adott eszközhöz tartozó összes attachment rekordot.")
  @ApiResponses({
  ...
  @Operation(
      summary = "Melléklet törlése",
      description =
          "DB rekord + fizikai fájl törlése. "
   ...
   *
   * <ul>
   *   <li>Ha {@code inline=false} (default): {@code Content-Disposition: attachment} — a böngésző
   *       letölti a fájlt az eredeti néven
   *   <li>Ha {@code inline=true}: {@code Content-Disposition: inline} — a böngésző megpróbálja
   *       megjeleníteni (image, PDF iframe, stb.)
   * </ul>
   *
   * <p>A {@code Content-Type} a tárolt {@code mime_type} mezőből jön. A Content-Disposition header
   * UTF-8 filename kódolást használ (RFC 5987).
   */
  @Operation(
      summary = "Melléklet letöltése / preview",
      description =
          "Visszaadja a fájl bináris tartalmát. "
  ...
  public ResponseEntity<byte[]> downloadFile(
      ...
      @Parameter(
              description =
                  "Ha true, inline preview (Content-Disposition: inline); ha false, letöltés (attachment)")
