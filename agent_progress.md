...
- [ ] **Task 1.3:** Flyway `V1__init_schema.sql` megírása (11 tábla: configs, permissions, roles, locations [version mezővel], app_users [must_change_password, password_changed_at, failed_login_count, locked_until], softwares [license_key_encrypted], devices [status ENUM], device_attachments [mime_type max 100, size_bytes, storage_path ./uploads/devices/{id}/{uuid}.{ext}], device_assignments [egyetlen tábla, service assert unassign_* NULL], audit_logs [entity_type, entity_id, http_status], refresh_tokens). **Classpath:** `backend/src/main/resources/db/migration/`. **`flyway.baseline-on-migrate: false`** (tiszta indulás, manuális baseline parancs szükséges ha nem üres a DB). **`spring.flyway.repair-on-migrate: false`** (biztonságos: checksum mismatch esetén hiba, manuális repair parancs). **Nincs Flyway undo** — rollback az audit log changes_json-ból. **Depends on:** 1.2
- [ ] **Task 1.4:** Flyway `V2__seed.sql` (14 permission [beleértve LOCATION_READ], 3 role + mapping, 12 configs [JWT_KID_GRACE_PERIOD_SEC=3600, ALERT_EMAIL_RECIPIENT], demo admin user **pre-hashed Argon2 string** `ChangeMe123!` jelszóhoz [memory=65536, iterations=3, parallelism=1] + must_change_password=true). **Depends on:** 1.3
- [ ] **Task 1.5:** JPA Entitások + `BaseEntity` abstract class (`@MappedSuperclass`, `id`, `created_at`, `updated_at` JPA Auditing által töltve, `@EnableJpaAuditing` a backend-en) + LocationService.validateNoCycle() rekurzív ciklus-ellenőrzéssel + @Version optimistic lock (`LocationService.move()` `@Transactional`, **OptimisticLockException esetén 3x retry** külön tranzakcióban) + cascade delete az attachments-re device törléskor. **Depends on:** 1.3
